...
GATE_TABLE = {**STANDARD_GATE_TABLE, **COMPOSITION_GATE_TABLE}
...
COMPOSITION_CX_TABLE = {"cz":1, "cy":1, "swap":3, "ch":2, "ccx":6, "cswap":8, "crx":2, "cry":2,
        ...
        "c4x":18}
CX_TABLE = {**STANDARD_CX_TABLE, **COMPOSITION_CX_TABLE}
...
def function_gate(line, line_id):
    ...
    if op in function_table: # User-defined gate function
        ...
        for p in paramlist_ga:
            s += "\tsim.append(Simulation::" + op.upper() + "(" + p + "));\n"
...
# svfile.write('#include "utils.cuh"\n')
svfile.write('#include "bmlqsim.cuh"\n')
#svfile.write('#include "svsim_nvgpu_omp.cuh"\n')

svfile.write('//Use the SVSim namespace to enable C++/CUDA APIs\n')
svfile.write('using namespace SVSim;\n\n')
...
chunk_size = nqubits - 5 if nqubits - 5 <= 25 else 25

svfile.write('int main()\n{\n')
svfile.write('\tsrand(RAND_SEED);\n')
svfile.write('\tint n_qubits=' + str(nqubits) + ';\n')
svfile.write('\tint n_gpus=2;\n')
svfile.write('\tint chunkSize=' + str(chunk_size) + ';\n')
svfile.write('\tSimulation sim(n_qubits, n_gpus, chunkSize);\n')
svfile.write('\tprepare_circuit(sim);\n')
svfile.write('\tsim.beginSimulation();\n')
# svfile.write(f'\twrite_array_to_binary<double>("{args.output[:-3]}_real.bin", sim.svRealPtrHost, exp2Int(n_qubits));\n')
# svfile.write(f'\twrite_array_to_binary<double>("{args.output[:-3]}_imag.bin", sim.svImagPtrHost, exp2Int(n_qubits));\n')
